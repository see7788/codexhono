...
- handler 只负责读取请求、校验输入、调用业务对象或 store action、返回响应；复杂业务流程不要堆在 route handler 里，业务边界不存在时按 scope-style「后端作用域」或 zustand-store-style「后端仓库」建最小业务对象或 action。
...
- 页面交互、组件职责和 UI 临时态使用 scope-style「前端作用域」；业务状态流转使用 zustand-store-style「前端仓库」。
...
- SSE 数据写入业务状态时，前端使用 zustand-store-style「前端仓库」，后端使用 zustand-store-style「后端仓库」。
...
- WebSocket 消息写入业务状态时，前端使用 zustand-store-style「前端仓库」，后端使用 zustand-store-style「后端仓库」。
