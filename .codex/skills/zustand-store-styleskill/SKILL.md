...
- 单切片主仓库直接组合切片工厂：`create<Store>()(immer(createSliceStore))`；禁止为了传递 set/get/api 再包一层 wrapper、展开对象或制造中间 Store 类型。
- 只有多个切片合并、persist 生命周期或真实组合逻辑存在时，才写主仓库组合函数；无真实组合逻辑时保持单行组合。
...
- 只有跨文件公共行为、维护对象不变量或多个入口共用逻辑才进入 `${dir}Actions`；路由私有流程、单点响应类型和单点 helper 不进入切片仓库。
- 非持久化运行时状态放在切片文件级变量；只有确实需要跨 action 或跨入口访问时，才通过 `${dir}Actions` 暴露 `xxGet/xxSet`。
- 持久化只保存真正需要恢复的业务数据；窗口句柄、连接对象、stream、队列、临时登录状态等进程运行时资源不持久化。
...
- 服务端切片只返回 `{ ${dir}, ${dir}Actions }`；`${dir}` 是对象状态，`${dir}Actions` 是跨文件公共行为。
...
- zustand + immer 出现 set/get/api、StateCreator、never、unknown、隐式 any 类型错误时，先检查 middleware mutator 泛型和切片工厂返回边界。
- 如果切片工厂来自 workspace 外部包源码，必须检查该源码包的 peer dependency 是否能被当前 tsconfig 解析；必要时用 `tsc --traceResolution` 定位解析失败源。
- 禁止用 `as` 修复 store 类型；只有定位到真实类型边界后，才调整切片工厂泛型、主仓库组合写法或最小 tsconfig paths 映射。

## Action

- action 表达业务动作并与业务语义同名；普通业务状态不暴露 `stateGet`、`stateSet` 这类包一层的基础 API。
- 只有非持久化文件级变量需要跨 action 或跨入口读写时，才允许在 action 中提供 `xxGet/xxSet`；文件私有读取和写入留在当前文件。
