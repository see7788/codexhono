    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "只有外部协议确实要求环境变量且无法改用有类型配置时，才允许在最小真实消费作用域建立 `env-meta`：仅一个子包使用时放在该子包自己的实现目录内；两个及以上子包真实共用时，才提升为 pnpm 根目录直属的 `<pnpm-root>/env-meta` workspace 子包并由 `pnpm-workspace.yaml` 显式纳入。没有真实需求不得预建，也不得为未来复用提前提升。",
          "只有归属明确的 `env-meta` 边界内部可以直接读写 `process.env`；入口、runtime、config 子包、其他业务目录、应用、helper、wrapper、adapter、class、私有方法、测试和脚本均无例外，移动代码位置、借 dotenv 或动态属性访问不构成合规。同一环境字段只能有一个 owner，共用后必须提升并删除原有重复实现。",
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "`env-meta` 的目录层级必须表达真实作用域：只有一个子包消费时，放在该子包现有实现目录下并保持私有；出现两个及以上已确认的跨子包消费点时，才提升为 `<pnpm-root>/env-meta` 独立 workspace 子包并由 `pnpm-workspace.yaml` 显式纳入。提升时迁移唯一实现并删除原私有副本，禁止根目录与子包同时维护同一环境字段。",
