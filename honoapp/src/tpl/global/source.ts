...
不得新增 *.test.*、*.spec.*、__tests__/ 或其他仅供测试的文件；默认以现有构建、类型检查、真实接口或页面运行验证交付。只有老板明确要求测试或仓库既有测试体系且改动必须接入时才新增。
...
"""`,
    ...
    sections: [
      {
        title: "总纲",
        items: [
          "每项工作开始前，必须确认 `worker`、`workerLow`、`tokener` 三个全局定义可用：项目级 `.codex/agents/` 优先，缺失时才回退用户级 agents。三项定义缺一不可，缺失时必须立即告知用户并停止工作；按任务选择实际参与的角色：`workerLow` 用于轻量、边界明确的调查、检查和局部修改，`worker` 用于主力实现、调试、测试和较完整调查，`tokener` 用于关键 review、teacher 式指导或解除 `worker` 阻塞，默认不修改。三者不构成固定流水线，也不要求同时参与。",
      ...
      },
      {
        title: "Agents 兜底定义",
        items: [
          "`worker`、`workerLow`、`tokener` 是全局可用的工作者定义；项目级 `.codex/agents/` 优先，不存在时才回退用户级定义。",
          "按任务选择实际参与的角色：`workerLow` 处理轻量、边界明确的调查、检查和局部修改；`worker` 处理主力实现、调试、测试和较完整调查；`tokener` 处理关键 review、teacher 式指导或解除 `worker` 阻塞，默认不修改。三者不构成固定流水线，也不要求同时参与。",
  ...
  skills: {
      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            "项目自定义函数、方法、构造器和 store action 只有在跨层/跨包、多个独立调用者、可选字段组合或稳定请求契约存在时，才使用对象形参；单一调用点、同文件且没有独立业务语义的参数组合保持直接形参，即使有两个及以上业务值也不为形式创建对象包裹、Input/DTO 或 helper。对象形参类型优先从完整领域类型用 Pick/Omit 派生，只有多个真实消费点时才允许命名 type/interface。框架或第三方规定的回调签名、rest 参数除外。",
      ...
      sections: [
        {
          title: "分流规则",
          items: [
            "每项技术工作开始前先检查当前会话已暴露的 MCP 工具，并按任务选择可用 MCP；需要的 MCP 未加载、不可用或需要重启时如实记录并告知老板，不得假装已使用或静默用臆测替代。默认使用 Codegraph 作为代码库主力 MCP；改代码前的源码定位、调用关系、调用路径和影响范围分析优先走 Codegraph。",
    ...
    [nodes.checklistStyle]: {
      description: "涉及任务拆解、进度状态、阻塞同步和多阶段验证时使用。用户提到任务台账、待办事项、todolist 或 todoclick 时必须使用，并在相关文档末尾维护统一任务台账。约束触发条件、状态标记、决策记录、worker/workerLow/tokener 真实 agent 命名和完成表达。",
      title: "可审计的工作流",
      sections: [
        {
          title: "文本完整性验收",
        ...
        },
        {
          title: "触发规则",
          items: [
            "在任何分析、诊断、实现或验证前，必须确认 `worker`、`workerLow`、`tokener` 的项目级定义，项目级不存在时回退用户级定义；三者任一缺失、不可用或无法实际创建时，必须在“可审计的工作流”中写为 `[!]`、告知用户并立即停止；三者定义完备时，按任务选择实际参与的角色，不要求固定流水线或三者同时参与。",
        ...
        },
        {
          title: "状态同步",
          items: [
            "台账默认以可独立验收的交付物为一行，不拆成普通实现步骤；状态只使用：`[?]` 待确认、`[ ]` 待办、`[>]` 未派工、`[~]` 运行中、`[<]` 已反馈、`[|]` 已中断、`[x]` 已完成、`[!]` 阻塞、`[-]` 已取消。",
            ...
            "文档中的正式标题固定为 `## 可审计的工作流 [?] 待确认、[ ] 待办、[>] 未派工、[~] 运行中、[<] 已反馈、[|] 已中断、[x] 已完成、[!] 阻塞、[-] 已取消`。每项使用单独一行的无序列表，以状态符号开头并用正常语句说明任务、涉及范围和完成依据；不强制拆成固定字段。完成项必须在同一行写明真实验证证据。",
            "每条任务在状态符号后只记录该独立交付物实际参与的 agent 角色与运行时实际模型标识，以及该角色的真实结果。`workerLow` 的轻量调查、检查或局部修改，`worker` 的实现、调试、测试或较完整调查，`tokener` 的关键 review、teacher 式指导或解除阻塞，均按实际参与分别记录；`tokener` 默认不修改。它们不构成固定流水线，也不要求同时出现在同一条；只有同一独立交付物确实经历多个阶段时，才按时间顺序同一行记录。",
            ...
            "agent 后使用自然动作词描述事实，例如 `worker 处理`、`workerLow 检查`、`tokener review`、`worker 解决`；状态符号表示任务状态，动作词不替代状态。任务交接时在同一行写真实的 agent 变更，不得预先编造尚未参与的 agent。",
